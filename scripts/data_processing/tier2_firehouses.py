...
def process_firehouses(project_root: str | Path, force: bool = False, cache_mgr=None) -> dict:
    ...
    # Cache check
    if not force and cache_mgr and cache_mgr.is_valid("tier2_firehouses"):
        print("  [cache] Firehouse data unchanged -- using cached data.")
    ...
    geometry = [Point(xy) for xy in tqdm(
        zip(df_valid[lon_col], df_valid[lat_col]),
        total=len(df_valid), desc="  Creating geometry", leave=False
    )]
    ...
    if cache_mgr:
        cache_mgr.update("tier2_firehouses")
