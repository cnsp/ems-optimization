...
def build_distance_matrices(
    ...
    # Cache check
    if not force and cache_mgr and cache_mgr.is_valid("tier3_distance"):
        print("  [cache] Distance matrices unchanged -- using cached data.")
    ...
    metrics = list(tqdm(
        [("haversine", out_haversine), ("manhattan", out_manhattan)],
        desc="  Distance metrics",
        leave=False,
    ))

    for metric, out_path in metrics:
        dm = build_distance_matrix(fh, destinations, metric=metric)
        dm.to_csv(out_path)
        print(f"  Saved {out_path.name} ({dm.shape})")

    if cache_mgr:
        cache_mgr.update("tier3_distance")
