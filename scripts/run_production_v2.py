...
def generate_visualizations(df: pd.DataFrame, tables: Dict[str, pd.DataFrame]):
    ...
    ax.set_title(f"Policy Comparison: Mean Response Time vs Fleet Size\n(capacity={CAPACITY} units/firehouse, P0 Baseline)")
    ax.legend()
    ...
    for policy in POLICIES:
        ...
        ax2.plot(pdf["K"], pdf["coverage_10min"], marker="s",
                 label=policy, color=colors[policy], linewidth=2, markersize=6)
    ax0.set_title(f"6-Minute Coverage vs K (NYC Requirement, cap={CAPACITY})")
    ...
    for K in [20, 30, 40]:
        ...
        for patch, policy in zip(bp["boxes"], labels_for_box):
            ...
            patch.set_alpha(0.6)
        ax.set_ylabel("Mean Response Time (minutes)")
        ax.set_title(f"Response Time Distribution at K={K} (30 replications, cap={CAPACITY})")
    ...
    for policy in POLICIES:
        ...
        ax2.plot(pdf["K"], pdf["mean_queue_length"], marker="s",
                 label=policy, color=colors[policy], linewidth=2)
    ax1.set_title(f"Queue Fraction vs K (cap={CAPACITY})")
    ax1.set_xlabel("K"); ax1.set_ylabel("Fraction of Incidents Queued")
    ax1.legend(); ax1.grid(True, alpha=0.3); ax1.set_xticks(K_VALUES)
    ax2.set_title(f"Mean Queue Length vs K (cap={CAPACITY})")
    ax2.set_xlabel("K"); ax2.set_ylabel("Mean Queue Length")
    ax2.legend(); ax2.grid(True, alpha=0.3); ax2.set_xticks(K_VALUES)
    fig.suptitle(f"Queue Metrics by Fleet Size (capacity={CAPACITY} units/firehouse)", fontsize=13, y=1.02)
    fig.tight_layout()
    ...
    ax.set_title(f"Unit Utilization vs Fleet Size (cap={CAPACITY})")
    ...
    if not effect.empty:
        ...
        ax.set_title(f"Effect Sizes: Pairwise Policy Comparisons (cap={CAPACITY})")
